    ...
    def _get_results(self, params):
        if not params:
            return {
                ...
                'objects': [
                    {'id': 1, 'foo': 1, 'indexed_on': '2017-01-01T15:36:22Z'}
                ],
            }
        else:
    ...
    def _get_results(self, params):
        ...
        if not params:
            return {
                ...
                'objects': [
                    {'id': 1, 'foo': 1, 'indexed_on': '{}Z'.format(since1)}
                ],
            }
        else:
            since_query_param = DATE_PARAMS['indexed_on'].start_param
...
class TestCommCareHqClient(unittest.TestCase):
    ...
    def test_repeat_limit(self):
        with pytest.raises(
            ...
            self._test_iterate(
                FakeRepeatedDateCaseSession(),
                get_paginator('case', 2),
                2,
                [1, 2],
            )
    ...
    def test_message_log(self):
        self._test_iterate(
            FakeMessageLogSession(),
            get_paginator('messaging-event', 2),
            3,
            [1, 2, 3],
        )
...
class TestDatePaginator(unittest.TestCase):
    ...
    def test_multi_field_sort(self):
        ...
        self.assertEqual(
            paginator.get_since_date({'objects': [{'s1': d1, 's2': d2}]}),
            datetime.strptime(d1, '%Y-%m-%dT%H:%M:%SZ'),
        )

        self.assertEqual(
            paginator.get_since_date({'objects': [{'s2': d2}]}),
            datetime.strptime(d2, '%Y-%m-%dT%H:%M:%SZ'),
        )

        self.assertEqual(
            paginator.get_since_date({'objects': [{'s1': None, 's2': d2}]}),
            datetime.strptime(d2, '%Y-%m-%dT%H:%M:%SZ'),
        )
