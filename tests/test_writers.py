...
class TestSQLWriters(object):
    ...
    def _test_types(self, writer, table_name):
        ...
        with writer:
            ...
            result = dict(
                [
                    ...
                    for row in connection.execute(
                        'SELECT id, a, b, c, d, e FROM %s' % table_name
                    )
                ]
            )
            ...
            for id, row in result.items():
                ...
                assert dict(row) == self._type_convert(
                    connection, expected[id]
                )
    ...
    def test_explicit_types(self, strict_writer):
        ...
        assert len(result) == 2
        # a casts strings to ints, b casts ints to text, c default falls
        # back to ints, d default falls back to text
        assert dict(result['bizzle']) == {
            'id': 'bizzle',
            'a': 1,
            'b': '2',
        ...
        with writer:
            ...
            create_sql = sqlalchemy.schema.CreateTable(
                sqlalchemy.Table(
                    ...
                    metadata,
                    sqlalchemy.Column(
                        'id', sqlalchemy.NVARCHAR(length=100), primary_key=True
                    ),
                    sqlalchemy.Column(
