...
def _check_data(writer, expected, table_name, columns):
    actual = [
        list(row)
        for row in writer.engine.execute(
...
def _pull_mock_data(
    ...
    args = make_args(
        query=query, output_format='sql', start_over=start_over, since=since
    )

    assert not (
        checkpoint_manager and since
    ...
    def test_cli_database_error_checkpoint(
        ...
        runs = list(
            strict_writer.engine.execute(
                sqlalchemy.text(
                    'SELECT table_name, since_param, last_doc_id from commcare_export_runs where query_file_name = :file'
                ),
...
class TestCLIPaginationMode(object):
    ...
    def test_cli_pagination_since(self, writer, all_db_checkpoint_manager):
        ...
        # mode due to how the mock client is setup
        _pull_mock_data(
            writer,
            None,
            get_indexed_on_client(1),
