...
class TestCommCareMiniLinq(unittest.TestCase):
    ...
    def _test_eval(self, pagination_mode):
        form_order_by = get_paginator('form', pagination_mode=pagination_mode).since_field
        case_order_by = get_paginator('case', pagination_mode=pagination_mode).since_field
        ...
        client = MockCommCareHqClient({
            'form': [
                (
                    {'limit': 1000, 'filter': 'test1', 'order_by': form_order_by},
                    [1, 2, 3],
                ),
                (
                    {'limit': 1000, 'filter': 'test2', 'order_by': form_order_by},
                    [
                        { 'x': [{ 'y': 1 }, {'y': 2}] },
                        { 'x': [{ 'y': 3 }, {'z': 4}] },
                        { 'x': [{ 'y': 5 }] }
                ...
                (
                    {'limit': 1000, 'filter': 'laziness-test', 'order_by': form_order_by},
                    (i if i < 5 else die('Not lazy enough') for i in range(12))
                ),
                (
                    {'limit': 1000, 'cases__full': 'true', 'order_by': form_order_by},
                    [1, 2, 3, 4, 5]
                ),
            ],
            ...
            'case': [
                (
                    {'limit': 1000, 'type': 'foo', 'order_by': case_order_by},
                    [
                        { 'x': 1 },
                        { 'x': 2 },
                        { 'x': 3 },
