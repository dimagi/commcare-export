...
class TestCheckpointManager(object):
    ...
    def test_get_time_of_last_checkpoint(self, configured_manager):
        ...
        manager.set_checkpoint(datetime.datetime.utcnow(), PaginationMode.date_indexed)
    ...
    def test_clean_on_final_run(self, configured_manager):
        manager = configured_manager.for_dataset('form', ['t1'])
        manager.set_checkpoint(datetime.datetime.utcnow(), PaginationMode.date_indexed, doc_id="1")
        manager.set_checkpoint(datetime.datetime.utcnow(), PaginationMode.date_indexed, doc_id="2")
        ...
        manager.set_checkpoint(datetime.datetime.utcnow(), PaginationMode.date_indexed, True, doc_id="3")
    ...
    def test_multiple_tables(self, configured_manager):
        ...
        manager.set_checkpoint(last_run_time, PaginationMode.date_indexed, doc_id=doc_id)
...
@pytest.mark.parametrize('since, start_over, expected_since, expected_paginator', [
...
def test_checkpoint_details_static(since, start_over, expected_since, expected_paginator):
...
class TestCheckpointManagerProvider(object):
    def test_checkpoint_details_no_checkpoint(self, configured_manager):
        ...
        assert PaginationMode.date_indexed == CheckpointManagerProvider().get_pagination_mode(manager)

    def test_checkpoint_details_latest_from_db(self, configured_manager):
        manager = configured_manager.for_dataset('form', ['t1'])

        self._test_checkpoint_details(manager, datetime.datetime.utcnow(), PaginationMode.date_modified)
        self._test_checkpoint_details(manager, datetime.datetime.utcnow(), PaginationMode.date_indexed)
        self._test_checkpoint_details(manager, datetime.datetime.utcnow(), PaginationMode.date_modified)

    def _test_checkpoint_details(self, manager, checkpoint_date, pagination_mode):
