...
class TestMiniLinq(unittest.TestCase):
    ...
    def test_eval_reference(self):
        ...
        assert (
            Reference(Reference(Reference('a'))).eval(
                DictEnv({'a': 'b', 'b': 'c', 'c': 2})
            )
            == 2
        )
        ...
        self.check_case(
            Reference("foo.$.baz").eval(JsonPathEnv({'foo': [2], 'baz': 3})),
            [3],
        )
    ...
    def test_eval_auto_id_reference(self):
        ...
        self.check_case(
            Reference("foo.id").eval(JsonPathEnv({'foo': {'id': 2}})), ['2']
        )
    ...
    def test_eval_auto_id_reference_nested(self):
        ...
        # (see https://github.com/kennknowles/python-jsonpath-rw/pull/96)
        #
        #   Reference("id"):
        #       '1.bar.bazzer' -> 'bazzer'
        #
    ...
    def test_eval_collapsed_list(self):
        ...
        assert (
            Apply(Reference(">"), Literal(56), Literal(23.5)).eval(env) == True
        )
        ...
        assert (
            Apply(Reference("str2bool"), Literal('false')).eval(env) == False
        )
        ...
        assert Apply(Reference("str2date"), Literal('2015-01-01')).eval(
            env
        ) == datetime(2015, 1, 1)
        assert Apply(
            Reference("str2date"), Literal('2015-01-01T18:32:57')
        ...
        assert (
            Apply(Reference("str2date"), Literal(u'日'.encode('utf8'))).eval(
                env
            )
            == None
        )
        ...
        assert (
            Apply(Reference("format-uuid"), Literal(0xF00)).eval(env) == None
        )
        assert (
            Apply(Reference("format-uuid"), Literal('f00')).eval(env) == None
        )
        assert (
            Apply(
        ...
        assert (
            Apply(
                Reference("selected-at"), Literal('a b c'), Literal('1')
            ).eval(env)
            == 'b'
        )
        assert (
        ...
        assert (
            Apply(
                Reference("selected-at"), Literal('a b c'), Literal('5')
            ).eval(env)
            is None
        )
        assert (
            Apply(Reference("selected"), Literal('a b c'), Literal('b')).eval(
                env
            )
            is True
        )
        assert (
            Apply(
                Reference("selected"), Literal(u'a b 日本'), Literal('d')
            ).eval(env)
            is False
        )
        assert (
            Apply(
                Reference("selected"), Literal(u'a bb 日本'), Literal('b')
            ).eval(env)
            is False
        )
        assert (
        ...
        assert (
            Apply(Reference("default"), Literal(None), Literal('a')).eval(env)
            == 'a'
        )
        assert (
            Apply(Reference("default"), Literal('b'), Literal('a')).eval(env)
            == 'b'
        )
        assert (
            Apply(Reference("count-selected"), Literal(u'a bb 日本')).eval(env)
            == 3
        )
        assert (
            Apply(Reference("sha1"), Literal(u'a bb 日本')).eval(env)
            == 'e25a54025417b06d88d40baa8c71f6eee9c07fb1'
        )
        assert (
            Apply(Reference("sha1"), Literal(b'2015')).eval(env)
            == '9cdda67ded3f25811728276cefa76b80913b4c54'
        )
        assert (
            Apply(Reference("sha1"), Literal(2015)).eval(env)
            == '9cdda67ded3f25811728276cefa76b80913b4c54'
        )
    ...
    def test_or(self):
        ...
        assert (
            Apply(
                Reference("or"), Literal(1), Literal(laziness_iterator)
            ).eval(env)
            == 1
        )
        assert (
            Apply(
                Reference("or"), Literal(''), Literal(laziness_iterator)
            ).eval(env)
            == ''
        )
        assert (
            Apply(
                Reference("or"), Literal(0), Literal(laziness_iterator)
            ).eval(env)
            == 0
        )
        with pytest.raises(LazinessException):
            Apply(
                Reference("or"), Literal(None), Literal(laziness_iterator)
            ).eval(env)

        env = env | JsonPathEnv({'a': {'c': 'c val'}})
        assert (
            Apply(Reference("or"), Reference('a.b'), Reference('a.c')).eval(
                env
            )
            == 'c val'
        )
        assert (
            Apply(Reference("or"), Reference('a.b'), Reference('a.d')).eval(
                env
            )
            is None
        )
        ...
        self.check_case(
            Apply(Reference("or"), Reference('a.[*]'), Reference('b')).eval(
                env
            ),
            [1, 2],
        )
        self.check_case(
            Apply(Reference("or"), Reference('b.[*]'), Reference('c')).eval(
                env
            ),
            [1, 2],
        )
        self.check_case(
            Apply(Reference("or"), Reference('a.[*]'), Reference('$')).eval(
                env
            ),
            {'a': [], 'b': [1, 2], 'c': 2, 'id': '$'},
        )
    ...
    def test_attachment_url(self):
        env = BuiltInEnv(
            {'commcarehq_base_url': 'https://www.commcarehq.org'}
        ) | JsonPathEnv({'id': '123', 'domain': 'd1', 'photo': 'a.jpg'})
        expected = (
            'https://www.commcarehq.org/a/d1/api/form/attachment/123/a.jpg'
        )
        assert (
            Apply(Reference('attachment_url'), Reference('photo')).eval(env)
            == expected
        )
    ...
    def test_attachment_url_repeat(self):
        env = BuiltInEnv(
        ...
        ) | JsonPathEnv(
            {
                'id': '123',
                'domain': 'd1',
                'repeat': [{'photo': 'a.jpg'}, {'photo': 'b.jpg'}],
            }
        )
        expected = [
            'https://www.commcarehq.org/a/d1/api/form/attachment/123/a.jpg',
    ...
    def test_form_url(self):
        env = BuiltInEnv(
            {'commcarehq_base_url': 'https://www.commcarehq.org'}
        ) | JsonPathEnv({'id': '123', 'domain': 'd1'})
        expected = 'https://www.commcarehq.org/a/d1/reports/form_data/123/'
        assert (
            Apply(Reference('form_url'), Reference('id')).eval(env) == expected
        )
    ...
    def test_case_url(self):
        ...
        assert (
            Apply(Reference('case_url'), Reference('id')).eval(env) == expected
        )
    ...
    def test_unique(self):
        ...
        assert Apply(Reference('unique'), Reference('list[*].a')).eval(
            env
        ) == [1, 2, 3]
    ...
    def test_map(self):
        ...
        laziness_iterator = RepeatableIterator(
            lambda: (
                {'a': i} if i < 5 else die('Not lazy enough')
                for i in range(12)
            )
        )
        ...
        assert list(
            Map(
                source=Literal([{'a': 1}, {'a': 2}, {'a': 3}]), body=Literal(1)
            ).eval(env)
        ) == [1, 1, 1]
        assert list(
            Map(
                source=Literal([{'a': 1}, {'a': 2}, {'a': 3}]),
                body=Reference('a'),
            ).eval(env)
        ) == [1, 2, 3]
    ...
    def test_flatmap(self):
        ...
        laziness_iterator = RepeatableIterator(
            lambda: (
                {'a': range(i)} if i < 4 else die('Not lazy enough')
                for i in range(12)
            )
        )
        ...
        assert list(
            FlatMap(
                source=Literal([{'a': [1]}, {'a': 'foo'}, {'a': [3, 4]}]),
                body=Literal([1, 2, 3]),
            ).eval(env)
        ) == [1, 2, 3, 1, 2, 3, 1, 2, 3]
        assert list(
            FlatMap(
                source=Literal([{'a': [1]}, {'a': [2]}, {'a': [3, 4]}]),
                body=Reference('a'),
            ).eval(env)
        ) == [1, 2, 3, 4]
    ...
    def _setup_emit_test(self, emitter_env):
        env = (
            BuiltInEnv()
            | JsonPathEnv({'foo': {'baz': 3, 'bar': True, 'boo': None}})
            | emitter_env
        )
        Emit(
            table='Foo',
    ...
    def test_emit_mutli_different_query(self):
        ...
        writer = JValueTableWriter()
        env = BuiltInEnv() | JsonPathEnv() | EmitterEnv(writer)
        # the filter here is to prevent accumulating a `[None]` value
        # for each doc
        result = Filter(
                    ...
                    [
                        ...
                        Emit(
                            ...
                            source=Map(
                                ...
                                body=List([Reference("$.id"), Reference("a")]),
                            ),
                        ),
                    ]
                ),
