...
class TestExcelQuery(unittest.TestCase):
    ...
    def test_split_leftmost(self):
        assert split_leftmost(parse_jsonpath('foo')) == (
            jsonpath.Fields('foo'),
            jsonpath.This(),
        )
        ...
        assert split_leftmost(parse_jsonpath('[*].baz')) == (
            jsonpath.Slice(),
            jsonpath.Fields('baz'),
        )
        assert split_leftmost(parse_jsonpath('foo[*].baz')) == (
    ...
    def test_get_safe_source_field(self):
        assert _get_safe_source_field('foo.bar.baz') == Reference(
            'foo.bar.baz'
        )
        assert _get_safe_source_field('foo[*].baz') == Reference('foo[*].baz')
        assert _get_safe_source_field('foo..baz[*]') == Reference(
            'foo..baz[*]'
        )
        assert _get_safe_source_field('foo.#baz') == Reference('foo."#baz"')
        assert _get_safe_source_field('foo.bar[*]..%baz') == Reference(
        ...
        assert _get_safe_source_field('foo.bar:1.baz') == Reference(
            'foo."bar:1".baz'
        )
    ...
    def test_compile_mapped_field(self):
        ...
        expression = compile_mapped_field(
            {'a': 'mapped from a'}, Reference('foo.bar')
        )
        assert expression.eval(env) == 'mapped from a'

        expression = compile_mapped_field(
            {'a': 'mapped from a'}, Reference('foo.baz')
        )
        assert list(expression.eval(env))[0].value == 'b'

        expression = compile_mapped_field(
            {'a': 'mapped from a'}, Reference('foo.boo')
        )
    ...
    def test_alternate_source_fields(self):
        minilinq = List(
            [
                # First sheet uses a CSV column and also tests combining
                # "Map Via"
                Bind(
                    'checkpoint_manager',
                    Apply(
                ...
                # Second sheet uses multiple alternate source field
        ...
        minilinq = List(
            [
                Bind(
                    ...
                    Filter(
                        predicate=Apply(
                            Reference("filter_empty"), Reference("$")
                        ),
                        source=Map(
                            source=Apply(
                                Reference("api_data"),
                ...
                Bind(
                    ...
                    Emit(
                        ...
                        source=Map(
                            ...
                            body=List(
                                [Reference("id"), Reference("$.user_id")]
                            ),
                        ),
                    ),
                ),
