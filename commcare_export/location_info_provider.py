...
class LocationInfoProvider:
    ...
    def get_location_types(self):
        ...
        location_type_dict = {}
        for row in self._api_client.iterate(
            'location_type', paginator, {'limit': self._page_size}
        ):
    ...
    def get_location_hierarchy(self):
        ...
        location_data = {}
        for row in self._api_client.iterate(
            'location', paginator, {'limit': self._page_size}
        ):
        ...
        for resource_uri in location_data:
            ...
            while loc_uri is not None:
                ...
                if loc_type not in self.location_types:
                    logger.warning(
                        'Unknown location type referenced: {}'.format(loc_type)
                    )
                    break
