...
def without_empty_tail(cells):
    ...
    return list(
        reversed(
            list(
                drop_while(lambda v: (not v) or (not v.value), reversed(cells))
            )
        )
    )
...
def compile_fields(worksheet, mappings=None):
    ...
    source_fields = extended_to_len(
        len(fields), get_column_by_name(worksheet, 'source field') or []
    )
    map_vias = extended_to_len(
        len(fields), get_column_by_name(worksheet, 'map via') or []
    )
    format_vias = extended_to_len(
        len(fields), get_column_by_name(worksheet, 'format via') or []
    )
...
def compile_source(worksheet, value_or_root=False):
    ...
    :return: tuple of the 'data source' expression and the
        'root doc expression':
    ...
    # out the first field
    if (
        not isinstance(data_source, jsonpath.Fields)
        or len(data_source.fields) > 1
    ):
    ...
    if required_column is None:
        ...
        for i in range(len(output_headings)):
            if output_headings[i].value == required_column.name.v:
                if (
                    isinstance(output_fields[i], Reference)
                    and output_fields[i].ref == required_column.source
                ):
                    extend_fields = False
                    continue
                else:
                    raise Exception(
                        f'Field name "{required_column.name.v}" conflicts '
                        f'with an internal name.'
                    )
...
def parse_sheet(
    worksheet, mappings=None, column_enforcer=None, value_or_root=False
):
...
def get_multi_emit_query(source, sheets, missing_value):
    ...
    data_source = sheets[
        0
    ].data_source  # sheets will all have the same datasource
    return Bind(
        'checkpoint_manager',
        Apply(
