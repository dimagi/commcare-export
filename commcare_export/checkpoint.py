...
class CheckpointManager(SqlMixin):
    ...
    def set_checkpoint(
        self, checkpoint_time, pagination_mode, is_final=False, doc_id=None
    ):
        self._set_checkpoint(
            checkpoint_time, pagination_mode, is_final, doc_id=doc_id
        )
        if is_final:
            self._cleanup()
    ...
    def get_latest_checkpoints(self):
        ...
        * key
        ...
        with session_scope(self.Session) as session:
            ...
            # Can't use this since MySQL < 8.0 doesn't support window
            # functions. Keeping for future reference
            #
            # window_func = (
            #     func.row_number()
            #     .over(
            ...
            # query = (
            #     session.query(Checkpoint)
            #     .select_entity_from(inner_query)
...
class CheckpointManagerProvider(object):
    def __init__(
        self, base_checkpoint_manager=None, since=None, start_over=None
    ):
    ...
    def get_checkpoint_manager(self, data_source, table_names):
        """
        This gets called before each table is exported and set in the
        ...
        if pagination_mode != PaginationMode.date_indexed:
            logger.warning(
                ...
                "To switch to the new mode you must re-sync your data using "
                "`--start-over`.\n"
