...
CLI_ARGS = [
    ...
    Argument(
        'password',
        help='Enter password, or if using apikey auth-mode, enter the api '
             'key.',
    ),
    Argument(
        'auth-mode',
    ...
    Argument(
        ...
        default=False,
        action='store_true',
        help='When saving to a SQL database; the default is to pick up since '
             'the last success. This disables that.',
    ),
    Argument('profile'),
        ...
        if output.startswith('mysql'):
            ...
            if len(charset_split) > 1 and charset_split[1] != 'utf8mb4':
                raise Exception(
                    ...
                    f"with the export. It is recommended that you use "
                    f"'utf8mb4' instead."
                )
...
def _get_checkpoint_manager(args):
    if (
        not args.users
        and not args.locations
        and not os.path.exists(args.query)
    ):
        logger.warning(
