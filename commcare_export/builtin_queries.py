...
# A MiniLinq query for internal CommCare user table. It reads every
# field produced by the /user/ API endpoint and writes the data to a
# table named "commcare_users" in a database.
...
# A MiniLinq query for internal CommCare location table. It reads every
# field produced by the /location/ API endpoint and appends fields to
# hold parent locations using location_type information before writing
# the data to a table named "commcare_locations" in a database.
...
def get_locations_query(lp):
    ...
    # method. Replace hyphens with underscores to be easier to use in
    # SQL.
...
class ColumnEnforcer():
    columns_to_require = {
        'form': Column('commcare_userid', '$.metadata.userID'),
