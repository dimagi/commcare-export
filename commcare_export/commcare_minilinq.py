                ...
                'filter': {
                    "or": [
                        ...
                        },
                        {
                            "and": [
                                server_modified_missing,
                                {"range": {"received_on": range_expression}},
                            ]
                        },
                    ]
                }
...
DATE_PARAMS = {
    ...
    'server_date_modified': SimpleSinceParams(
        'server_date_modified_start', 'server_date_modified_end'
    ),
    # used by messaging-events
    'date_last_activity': SimpleSinceParams(
        'date_last_activity.gte', 'date_last_activity.lt'
    ),
}


def get_paginator(
    resource, page_size=1000, pagination_mode=PaginationMode.date_indexed
):
    return {
...
class DatePaginator(SimplePaginator):
    ...
    def __init__(self, since_field, page_size=1000, params=DEFAULT_PARAMS):
        params = (
            DATE_PARAMS[since_field]
            if params is DatePaginator.DEFAULT_PARAMS
            else params
        )
