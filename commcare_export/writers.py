...
class SqlTableWriter(SqlMixin, TableWriter):
    ...
    def best_type_for(self, val):
        ...
        if isinstance(val, int):
            ...
            if self.is_postgres:
                ...
                # possible.
                if (
                    len(val) < self.MAX_VARCHAR_LEN
                ):  # FIXME: Is 255 an interesting cutoff?
    ...
    def make_table_compatible(self, table_name, row_dict, data_type_dict):
        ...
        for column, val in row_dict.items():
            ...
            if column not in columns:
                logger.warning(f"Adding column '{table_name}.{column} {ty}'")
                op.add_column(
    ...
    def write_table(self, table: TableSpec):
